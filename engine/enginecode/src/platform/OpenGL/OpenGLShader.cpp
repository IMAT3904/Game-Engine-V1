...
namespace Engine {
	...
	OpenGLShader::OpenGLShader(const char * vertexFilepath, const char * fragmentFilepath)
	{
		...
		{
			Log::error("Could not open shader vertex source: {0}", fragmentFilepath);
		}
		handle.close();
	...
	{
		enum Region{None = -1, Vertex = 0, Fragment, Geomentry, TesselationControl, TessellationEvalution, Compute} ;
		std::string line;
		...
		if (handle.is_open())
		{
			...
			{ 
				...
				if (line.find("#region Geomentry") != std::string::npos){ region = Region::Geomentry; continue; }
				if (line.find("#region TessellationControl") != std::string::npos) { region = Region::TesselationControl; continue; }
				if (line.find("#region TessellationEvaluation") != std::string::npos) { region = Region::TessellationEvalution; continue; }
